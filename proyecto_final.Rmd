...
```{r}
cuja_sf <- st_as_sf(gcuja$data, coords = c("decimalLongitude", "decimalLatitude"), 
...
Pueden visualizarse los gráficos para saber cuál es la que concentra mayores registros:
```{r}
cuja_regiones %>%
...
La comuna Chaitén es la que tiene mayor cantidad de avistamientos.
...
#Precipitaciones
```{r datos-pp, echo=TRUE}
cuja_climpp <- getData("worldclim", var = "prec", res = 2.5) |>
    crop(cuja_sf)

plot(cuja_climpp)

pp <- raster::extract(cuja_climpp, cuja_sf) |>
    as.data.frame()
...
#Temperatura
```{r datos-tmp, echo=TRUE}
cuja_climtmp <- getData("worldclim", var = "prec", res = 2.5) |>
    crop(cuja_sf)

plot(cuja_climtmp)

tmp <- raster::extract(cuja_climpp, cuja_sf) |>
    as.data.frame()
...
#Bioclimaticos
```{r datos-pp, echo=TRUE}
cuja_bioclim <- getData("worldclim", var = "prec", res = 2.5) |>
    crop(cuja_sf)

plot(cuja_bioclim)

bioclim <- raster::extract(cuja_climpp, cuja_sf) |>
    as.data.frame()

kableExtra::kable(bioclim) |> 
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed")) |>
  kableExtra::scroll_box(width = "100%", height = "400px")
```
...
bioclim <- bioclim_data

if (length(bioclim) > 0) {
  selected_layers <- 1:10 
  bioclim_selected <- bioclim[, selected_layers]
  
  bioclim_df <- cbind(cuja_sf, bioclim_selected)
  names(bioclim_df)[3:(ncol(bioclim_df) - 2)] <- paste0("Bioclim_", selected_layers)
  
  cuja_database <- cbind(bioclim_df, temperature, precipitation)
...
```{r}
rgt_region <- cuja_regiones %>%
  group_by(stateProvince, year) %>%
  count() %>%
  ungroup()

rgt_region <- rgt_region %>%
  arrange(stateProvince, year)

continuidad <- rgt_region %>%
